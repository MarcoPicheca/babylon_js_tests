...
var Tools = class _Tools {
  ...
  static set PerformanceLogLevel(level) {
    if ((level & _Tools.PerformanceUserMarkLogLevel) === _Tools.PerformanceUserMarkLogLevel) {
      if (_native?.enablePerformanceLogging) {
        ...
        _Tools.StartPerformanceCounter = _Tools._StartMarkNative;
        _Tools.EndPerformanceCounter = _Tools._EndMarkNative;
      } else {
  ...
  static _StartMarkNative(counterName, condition = true) {
    if (condition && _native?.startPerformanceCounter) {
      if (_Tools._NativePerformanceCounterHandles.has(counterName)) {
        _Tools.Warn(`Performance counter with name ${counterName} is already started.`);
      } else {
        const handle = _native.startPerformanceCounter(counterName);
        _Tools._NativePerformanceCounterHandles.set(counterName, handle);
      }
    }
  }
  static _EndMarkNative(counterName, condition = true) {
    if (condition && _native?.endPerformanceCounter) {
      const handle = _Tools._NativePerformanceCounterHandles.get(counterName);
      if (handle) {
        _native.endPerformanceCounter(handle);
        _Tools._NativePerformanceCounterHandles.delete(counterName);
      } else {
        _Tools.Warn(`Performance counter with name ${counterName} was not started.`);
      }
    }
  }
  /**
...
//# sourceMappingURL=chunk-MSDW7SPX.js.map
